...
DOC_META_KEYS = {"docHidden", "_doc_hidden", "_docHide", "_doc", "_custom_rows", "_custom_rows_before"}
        ...
        def flatten(data, parent_key='', schema=schema, hidden=False, root_schema=None, condition_data=None):
            ...
            if isinstance(data, dict):
                if 'value' in data:
                    ...
                    # Container Node
                    # UPGRADE: Append custom rows before recursing children
                    custom_rows_before = data.get('_custom_rows_before', [])
                    if isinstance(custom_rows_before, list):
                        for custom_row in custom_rows_before:
                            if isinstance(custom_row, list) and len(custom_row) == 4:
                                rows.append(custom_row)
                            else:
                                self.warning(f"Invalid custom row format: {custom_row}. Must be a list of 4 elements.")
                    ...
                    # Append custom rows after the subtree
                    custom_rows = data.get('_custom_rows', [])
